...
function loadData() {
    const heroDataContent = fs.readFileSync(path.join(ROOT, 'hero-data.js'), 'utf8');
    const heroMatch = heroDataContent.match(/var\s+HEROES_DATA\s*=\s*(\[[\s\S]*?\]);/);
    if (!heroMatch) {
        throw new Error('Не удалось прочитать HEROES_DATA');
    }

    const itemsDbContent = fs.readFileSync(path.join(ROOT, 'items-db.js'), 'utf8');
    const itemsMatch = itemsDbContent.match(/const\s+itemsDB\s*=\s*(\{[\s\S]*?\});/);
    if (!itemsMatch) {
        throw new Error('Не удалось прочитать itemsDB');
    }

    return {
        HEROES_DATA: JSON.parse(heroMatch[1].replace(/'/g, '"')),
        itemsDB: JSON.parse(itemsMatch[1].replace(/([A-Za-z0-9_]+)\s*:/g, '"$1":').replace(/'/g, '"'))
    };
}
...
function validateAdditionalPages() {
    console.log('\n--- Проверка дополнительных страниц ---');

    const extraPages = ['map.html', 'guide-druid.html', 'heroes/druid-nav-demo.html'];
...
function main() {
    ...
    try {
        ...
        if (!Array.isArray(HEROES_DATA)) {
            throw new Error('HEROES_DATA не загрузился');
        }
        if (!itemsDB || typeof itemsDB !== 'object') {
            throw new Error('itemsDB не загрузился');
        }

        validateHeroes(HEROES_DATA);
        validateItems(itemsDB);
        validateNavigation();
        validateAdditionalPages();
