// balance-dashboard-page.js — логика страницы balance-dashboard.html
// Зависимости (defer): hero-data.js, abilities-db.js, ability-stats.js, game-constants.js
...
(function() {
    ...
    function abilityLevel(heroLevel, reqLevel, maxLevels) {
        ...
        if (maxLevels <= 0) return Math.min(Math.floor(heroLevel / 60) + 1, 5);
        var lvl = Math.floor((heroLevel - reqLevel) / 8) + 1;
        return Math.min(lvl, maxLevels);
    }

    function heroStatsAtLevel(h, level) {
        var l = level - 1;
        var C = typeof GAME_CONSTANTS !== 'undefined' ? GAME_CONSTANTS : {};
        return {
            str: h.strBase + h.strGain * l,
            agi: h.agiBase + h.agiGain * l,
            int: h.intBase + h.intGain * l,
            hp: h.hp + h.strGain * l * (C.STR_HP_FACTOR || 20),
            def: h.def + h.agiGain * l * (C.AGI_ARMOR_FACTOR || 0.05),
            speed: h.speed,
            range: h.range
        };
    }
    ...
    function calcRawProfile(hero, level) {
        ...
        abilCodes.forEach(function(rc) {
            ...
            // Мобильность: дэш = дальность/кулдаун
            if (abS.dash) {
        ...
        var armorC = (typeof GAME_CONSTANTS !== 'undefined' && GAME_CONSTANTS.ARMOR_C) || 0.05;
        var ehp = (stats.hp + buffHP) * (1 + totalArmor * armorC) * (1 + evasion / 100);

        // Мобильность: скорость + дэш
    ...
    function populateSelects() {
        ...
        sortHeroesByName(heroes).forEach(function(h) { opts += '<option value="' + h.heroId + '">' + h.name + '</option>'; });
        s2.innerHTML = opts;
        s3.innerHTML = opts;
        var opts1 = '';
        sortHeroesByName(heroes).forEach(function(h) { opts1 += '<option value="' + h.heroId + '">' + h.name + '</option>'; });
        s1.innerHTML = opts1;
        s1.value = 'paladin';
        updateSelectOptions();
    }
